...
function parseRange(range: string, toFind: string): number[] {
  return range
    .replace(toFind, "")
    .split("-")
...
function partOne(inputs: string[]): number {
  return inputs.reduce((acc, input) => {
    const [range, password] = parse(input);
    const toFind = range.slice(-1);
    const [min, max] = parseRange(range, toFind);

    const occurences = password
      .slice()
      .split("")
      .reduce((acc, value) => (value === toFind ? acc + 1 : acc), 0);

    const validPassword = occurences >= min && occurences <= max;
    return validPassword ? acc + 1 : acc;
  }, 0);
}
...
function partTwo(inputs: string[]): number {
  return inputs.reduce((acc, input) => {
    const [range, password] = parse(input);
    const toFind = range.slice(-1);
    const [first, second] = parseRange(range, toFind);
    const validPassword =
      (password[first - 1] === toFind && password[second - 1] !== toFind) ||
      (password[first - 1] !== toFind && password[second - 1] === toFind);
    return validPassword ? acc + 1 : acc;
  }, 0);
}
