...
const rules = bagsInput.split("\n");
const bags: Bag[] = rules.map(rule => {
    const [statement, ...holds] = rule.split(",");
    const [name] = statement.split(" bags");
    return {
        colour: name,
        holds: [statement, ...holds].flatMap(hold => ({
            colour: /(?<=\d )[a-z]* [a-z]* /.exec(hold)?.pop().trim() ?? '',
            holds: []
    }))
    }
})

function colours(bags: Bag[], bagsToFind: string[]): string[] {
    const a = bags.map((bag: Bag) => {
        const hasBag = bag.holds.some((b: Bag) => bagsToFind.includes(b.colour))
        return hasBag ? bag.colour : '';
    })
...
for (let i = 0; i < 10; i++) {
    bagsToFind = [...bagsToFind, ...colours(bags, bagsToFind)]
}

console.log(new Set(bagsToFind).size - 1)
