...
function validHcl(hcl = ""): boolean {
  return /^#[0-9A-F]{6}$/i.test(hcl);
}

function validEcl(ecl = ""): boolean {
  const validColours = ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"];
  return validColours.some((colour) => colour === ecl);
}

function validPid(pid = ""): boolean {
  return pid.length === 9 && !isNaN(parseInt(pid));
}

function allFieldsValid(passport: Passport): boolean {
  return (
...
const validPassports = passports
  ...
  .map((passport) => passport.map((p) => p.split(":")))
  .map(Object.fromEntries)
  .filter(allFieldsValid)
  .map((object) => requiredFields.every((field) => field in object))
  .filter(Boolean);
